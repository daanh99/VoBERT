...
def heuristic_options():
    options = dict()
    options['device'] = 'cuda' if torch.cuda.is_available() else 'cpu'
    options["with_cuda"] = True

    options["output_dir"] = "./output/tbird/heuristic/"
    ...
    options["per_element_masking"] = True
    options["evaluate_element_level"] = True
    options['semantic_embeddings'] = False
    options["threshold_on_valid"] = True
    options['parse_semantic'] = True

    options["log_file"] = "TBird.log"
    options["dataset"] = "TBird"
    options["unseen_heuristic"] = True
