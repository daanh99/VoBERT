...
def fixed_window(line, window_size, adaptive_window, seq_len=None, min_len=0):
    # Remove the last empty element caused by the last '|' with [:-1]
    line = [ln.split("!;!") for ln in line.split('!|!')[:-1]]
...
def generate_train_valid(data_path, window_size=20, adaptive_window=True,
    ...
    print("Num samples: " + str(len(data_iter)))
    print("num_session" + str(num_session))
    print("testsize: " + str(test_size))
    ...
    for line in tqdm(data_iter, total=len(data_iter)):
    ...
    # print(logkey_seq_pairs)

    print(len(logkey_seq_pairs))
    print(len(time_seq_pairs))
    print(test_size)
