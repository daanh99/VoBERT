...
class BERTTrainer:
    ...
    def __init__(self, bert: BERT, vocab_size: int,
                 ...
                 hypersphere_loss=False, semantic_mode=False):
        """
        ...
        self.model = BERTLog(bert, vocab_size, semantic_mode).to(self.device)
        ...
        self.criterion_semantic = nn.MSELoss(reduction='none')
        self.criterion_semantic_cosine = nn.CosineEmbeddingLoss(reduction='none')
    ...
    def iteration(self, epoch, data_loader, start_train):
        ...
        data_iter = tqdm(enumerate(data_loader), total=totol_length)
        ...
        for i, data in data_iter:
            ...
            if self.semantic_mode:
                if not self.cosine_sim_loss:
                    # Compute the raw mask loss using the semantic L1 loss function, if specified
                    # If is_logkey is False, set the raw loss to zero
                    mask_loss_raw = torch.tensor(0) if not self.is_logkey else self.criterion_semantic(mask_lm_output, data["bert_label"])

                    # Compute a boolean mask indicating which elements are non-padding
                    non_pad_mask = data["bert_label"].sum(2) != 0

                    # Select the non-padding elements of the mask loss tensor using the boolean mask
                    mask_loss_without_pad = mask_loss_raw[non_pad_mask]

                    # Compute the number of non-padding elements
                    num_non_padding = non_pad_mask.sum()

                    # Compute the mean of the non-padding elements of the mask loss tensor, if there are any
                    # If there are no non-padding elements, set the loss to zero
                    if num_non_padding > 0:
                        mask_loss = mask_loss_without_pad.mean()
                    else:
                        mask_loss = torch.tensor(0.0)
                else:
                    cosine_embedding_loss = nn.CosineEmbeddingLoss(reduction='none')
                    mask_lm_output_flattened = mask_lm_output.view(-1, mask_lm_output.shape[-1])
                    bert_label_flattened = data["bert_label"].transpose(1, 2).contiguous().view(-1, mask_lm_output.shape[-1])
                    non_pad_mask = (data["bert_label"].sum(2) != 0).view(-1)
                    mask_loss_raw = torch.tensor(0) if not self.is_logkey else cosine_embedding_loss(mask_lm_output_flattened, bert_label_flattened, torch.ones(mask_lm_output_flattened.shape[0]).to(self.device))
                ...
                mask_loss_raw = torch.tensor(0) if not self.is_logkey else self.criterion(mask_lm_output.transpose(1, 2), data["bert_label"])
                mask_loss = mask_loss_raw
        ...
        print("Epoch: {} | phase: {}, loss={}, lr={}".format(epoch, str_code, avg_loss, lr))
